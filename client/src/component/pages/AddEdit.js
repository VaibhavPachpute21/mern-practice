...
const AddEdit = () => {
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.post(`https://first-api-zvey.onrender.com/user`, {
        name: name,
        email: email,
        contact: contact
      });
  ...
  const updateUser = async () => {
    const res = await axios.put(`https://first-api-zvey.onrender.com/user/${location.state.userData.id}`, {
      name: name,
      email: email,
      contact: contact
    });
